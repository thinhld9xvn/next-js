...
function ProductsLayout({ pageContext, siteOptions, UpdateSiteOptions, intl }) {
    const router = useRouter();
    const {messages} = intl;
    const {siteOptions : mySiteOptions, productPageData, breadcrumbs} = pageContext;
    const {pageData, productsList} = productPageData;
    const {title, content, seo, featuredImage} = pageData.nodes[0];
...
export default connect(mapStateToProps, mapDispatchToProps)(injectIntl(ProductsLayout));
