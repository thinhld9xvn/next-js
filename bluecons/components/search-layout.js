...
import { getArticles, getFeaturedImage } from '@js_dir/utils/articleUtils';
import { CATEGORY_PER_PAGE } from '@constants/constants';
import TemplateEmptyList from './templates/template-empty-list';
import { Grid } from 'react-loader-spinner';
function SearchLayout({ pageContext, siteOptions, UpdateSiteOptions }) {
    const {headSchema, siteOptions : mySiteOptions, pageData, articlesList} = pageContext;
    const [loading, setLoading] = useState(true);
    const [paged, setPaged] = useState(1);
    const [total, setTotal] = useState(null);
    const [filteredData, setFilteredData] = useState(null);
    const [numPerPage, setNumPerPage] = useState(CATEGORY_PER_PAGE);
    const {title, name, featuredImage} = pageData;
    ...
    return (
        <>
            ...
            <main>
                <Banner data = {{thumbnail : bannerImage, title, name}} />
