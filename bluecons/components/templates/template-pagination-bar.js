...
export default function TemplatePaginationBar({ data }) {
        ...
        if ( total && numPerPage ) {
            if ( isDiff(curPaged.current, paged) || 
                ...
                if ( total > numPerPage ) {
                    ...
                    //console.log(PAG_FIRST_NUM_SHOWN, firstI, endI);
                    for (let number = 1; number <= PAG_FIRST_NUM_SHOWN && number <= pageCount; number++) {
    ...
    return (
        <>
            {total ? (
                <section className="paginate-wp pagi">
                    <div className="container">
                        <div className="paginate-inner">
                            <Pagination>
                                {itemsPag}
                            </Pagination>
                        </div>
