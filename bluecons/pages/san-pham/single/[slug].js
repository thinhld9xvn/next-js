...
export async function getStaticProps(context) {
  try {
    ...
    const {slug} = params;
    const path = concat(process.env.PRODUCTS_POST_TYPE_REWRITE, slug);
    ...
    const {articlePage} = await getArticlesPageData(process.env.PRODUCTS_POST_TYPE, slug);
...
export async function getStaticPaths() {
    ...
    const {articlesPathList} = await getArticlesPathsList(process.env.PRODUCTS_POST_TYPE);
