...
export async function getStaticProps(context) {
    try {
      ...
      const {slug} = params;
      const path = concat(process.env.PRODUCTS_TAX_REWRITE, slug);
      ...
      const termPageData = await getTermPageData(process.env.PRODUCTS_POST_TYPE, process.env.PRODUCTS_TAX, slug);
      return {
        props: {        
          pageContext : {
            headSchema,
...
export async function getStaticPaths() {
    ...
    const {getTaxonomiesList : paths} = await getTaxPathsList(process.env.PRODUCTS_TAX);
    travelsePaths(paths);
    return {
