...
export default function SingleDichVuPage({ pageContext }) {
    return (
...
export async function getStaticProps(context) {
  try {
    ...
    const {slug} = params;
    const path = concat(process.env.SERVICES_POST_TYPE_REWRITE, slug);
    ...
    const {articlePage} = await getArticlesPageData(process.env.SERVICES_POST_TYPE, slug);
...
export async function getStaticPaths() {
    ...
    const {articlesPathList} = await getArticlesPathsList(process.env.SERVICES_POST_TYPE);
